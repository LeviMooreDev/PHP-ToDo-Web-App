...
function authCheck()
{
    ...
    if (isset($_COOKIE["authentication-remember-key"])) {
        ...
        query("DELETE FROM `authentication_remember_keys` WHERE `expire` < CURRENT_DATE()");
        if (query("SELECT 1 FROM `authentication_remember_keys` WHERE `key_`='$keyHashed' AND `ip`='$ipHashed'")->num_rows === 1) {
